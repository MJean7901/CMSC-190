...
global {
   ...
   // int init_exp <- round(init_human_pop* exposure_rate);
   	int init_exp <- 5;
    float step <- 1 #minutes;
    geometry shape <- envelope(square(50 #m));
    
    int infected <- init_inf update: people count (each.is_infected);
	int susceptible <- init_human_pop - (init_inf + init_exp) update: people count (each.is_susceptible);
	int recovered <- 0 update: people count (each.is_recovered);
	int exposed <-init_exp update: people count (each.is_exposed);
...
// ... (existing code)
	...
	reflex end_simulation when: susceptible = init_human_pop{
		do pause;
	}
